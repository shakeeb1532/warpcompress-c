...
static size_t try_algo_zstd(const unsigned char *in, size_t in_len,
...
static void do_compress(void *arg) {
  ...
  /* Compress, score by speed (warm-up phase does weighted selection) */
  ...
  /* COPY fallback when compressed is not worthwhile */
...
int warp_compress_file(const char *in_path, const char *out_path, const warp_opts_t *opt) {
  ...
  uint32_t chunk = opt->chunk_bytes > 0 ? (uint32_t)opt->chunk_bytes
                                        : warp_pick_chunk_size(total);
  ...
  /* Output buffer capacity (max bound of all codecs) */
  ...
  if (fwrite(&hdr, sizeof(hdr), 1, fout) != 1) { perror("write hdr"); goto fail_compress; }

  warp_chunk_t *table = (warp_chunk_t*)calloc(n, sizeof(*table));
  if (!table) { fprintf(stderr,"table OOM\n"); goto fail_compress; }
  if (fwrite(table, sizeof(*table), n, fout) != n) { perror("write table"); goto fail_compress_table; }
  ...
  if (warm_n > 0) {
    ...
    if (!tp || !jobs) { fprintf(stderr,"warmup OOM\n"); if(tp)tp_destroy(tp); free(jobs); goto fail_compress_table; }
    ...
    long data_start = ftell(fout);
    for (int i=0;i<warm_n;i++) {
      if (!jobs[i].ok) { fprintf(stderr,"warm chunk %d failed\n", i); tp_destroy(tp); free(jobs); goto fail_compress_table; }
      ...
      if (jobs[i].out_algo != WARP_ALGO_ZERO) {
        if (fwrite(jobs[i].comp, 1, jobs[i].comp_len, fout) != jobs[i].comp_len) {
          perror("write warm chunk");
          tp_destroy(tp); free(jobs); goto fail_compress_table;
        }
  ...
  if (warm_n < (int)n) {
    ...
    for (int j=0;j<rest;j++) {
      uint32_t i = (uint32_t)(warm_n + j);
      if (!jobs2[j].ok) { fprintf(stderr,"compress chunk %u failed\n", i); tp_destroy(tp2); free(jobs2); goto fail_compress_table; }
      ...
      if (jobs2[j].out_algo != WARP_ALGO_ZERO) {
        if (fwrite(jobs2[j].comp, 1, jobs2[j].comp_len, fout) != jobs2[j].comp_len) {
          perror("write bulk chunk"); tp_destroy(tp2); free(jobs2); goto fail_compress_table;
        }
  ...
  if (fwrite(table, sizeof(*table), n, fout) != n) { perror("rewrite table"); goto fail_compress_table; }
  fseek(fout, 0, SEEK_SET);
  if (fwrite(&hdr, sizeof(hdr), 1, fout) != 1) { perror("rewrite hdr"); goto fail_compress_table; }

  /* Index trailer (optional) */
  ...
  if (do_idx) {
    ...
    for (uint32_t i=0;i<n;i++) {
      wix_entry_v1_t e = { .payload_off = table[i].offset,
                           .orig_len = table[i].orig_len,
                           .comp_len = table[i].comp_len,
                           .algo = table[i].algo };
...
int warp_decompress_file(const char *in_path, const char *out_path, const warp_opts_t *opt) {
  ...
  (void)wc_truncate_file(fout, hdr.orig_size); /* best-effort pre-size */
