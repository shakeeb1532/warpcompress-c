...
static void do_compress(void *arg) {
  ...
  size_t best_len = 0; int best_algo = WARP_ALGO_COPY;

  int cand[4]; int ccount=0;
  if (j->prefer_algo==0) {
    cand[ccount++]=WARP_ALGO_ZSTD;
#ifdef HAVE_LZ4
    cand[ccount++]=WARP_ALGO_LZ4;
#endif
#ifdef HAVE_SNAPPY
    cand[ccount++]=WARP_ALGO_SNAPPY;
#endif
  } else { cand[ccount++]=j->prefer_algo; }

  for (int k=0;k<ccount;k++) {
    int algo = cand[k]; size_t got=0; double t0=now_secs();
  ...
  if (best_len==0 || best_len >= j->len - (j->len>>6)) {
    ...
    best_algo = WARP_ALGO_COPY; best_len=j->len; j->secs=0.0;
  }
  j->comp=out; j->comp_len=best_len; j->out_algo=best_algo; j->ok=1;
}
...
static void do_decompress(void *arg) {
  ...
  if (j->ent.algo == WARP_ALGO_ZERO) {
    memset(j->buf, 0, j->ent.orig_len); j->ok=1; return;
  }
...
static int score_pick_algo(int mode, size_t in_len,
  ...
  struct { int algo; size_t clen; double mbps; } c[3] = {
    { WARP_ALGO_ZSTD, z_len, z_mbps },
    { WARP_ALGO_LZ4,  l_len, l_mbps },
    { WARP_ALGO_SNAPPY, s_len, s_mbps }
  };
  for (int i=0;i<3;i++) if (c[i].clen>0) {
    double ratio=(double)c[i].clen/(double)in_len;
    double s = (mode==WARP_AUTO_THROUGHPUT) ? c[i].mbps :
               (mode==WARP_AUTO_RATIO)      ? (1.0 - ratio)*1000.0 :
                                              c[i].mbps * (1.0 + 3.0*(1.0 - ratio));
    if (s>best) { best=s; best_algo=c[i].algo; }
  }
  return best_algo;
}
...
int warp_compress_file(const char *in_path, const char *out_path, const warp_opts_t *opt) {
  ...
  uint32_t chunk = opt->chunk_bytes ? opt->chunk_bytes : warp_pick_chunk_size(total);
  ...
  if (!in_pool || !out_pool) { fprintf(stderr,"pool OOM\n");
    if(in_pool) pool_destroy(in_pool); if(out_pool) pool_destroy(out_pool);
    fclose(fout); close(fd_in); return 1; }
  ...
  if (!table) { fprintf(stderr,"table OOM\n"); goto fail; }
  if (fwrite(table, sizeof(*table), n, fout) != n) { perror("write table"); goto fail_tbl; }
  ...
  if (warm_n > 0) {
    ...
    if (!tp || !jobs) { fprintf(stderr,"warmup OOM\n"); if(tp)tp_destroy(tp); free(jobs); goto fail_tbl; }

    for (int i=0;i<warm_n;i++) {
      size_t off=(size_t)i*chunk; size_t len=(off+chunk<=total)?chunk:(total-off);
    ...
    for (int i=0;i<warm_n;i++) {
      if (!jobs[i].ok) { fprintf(stderr,"warm chunk %d failed\n", i); tp_destroy(tp); free(jobs); goto fail_tbl; }
      ...
      if (jobs[i].out_algo != WARP_ALGO_ZERO) {
        if (fwrite(jobs[i].comp, 1, jobs[i].comp_len, fout) != jobs[i].comp_len) {
          perror("write warm chunk"); tp_destroy(tp); free(jobs); goto fail_tbl;
        }
  ...
  if (warm_n < (int)n) {
    ...
    if (!tp2 || !jobs2) { fprintf(stderr,"bulk OOM\n"); if(tp2)tp_destroy(tp2); free(jobs2); goto fail_tbl; }

    for (int j=0;j<rest;j++) {
      uint32_t i=(uint32_t)(warm_n+j);
      size_t off=(size_t)i*chunk; size_t len=(off+chunk<=total)?chunk:(total-off);
      jobs2[j]=(c_job_t){ .fd=fd_in, .offset=off, .len=len,
                          .prefer_algo=(locked_algo?locked_algo:WARP_ALGO_ZSTD),
                          .level=level, .idx=(int)i,
                          .in_pool=in_pool, .out_pool=out_pool, .out_cap=out_cap };
    ...
    for (int j=0;j<rest;j++) {
      ...
      if (!jobs2[j].ok) { fprintf(stderr,"compress chunk %u failed\n", i);
        tp_destroy(tp2); free(jobs2); goto fail_tbl; }
      long my_off=ftell(fout);
      ...
      if (jobs2[j].out_algo != WARP_ALGO_ZERO) {
        if (fwrite(jobs2[j].comp, 1, jobs2[j].comp_len, fout) != jobs2[j].comp_len) {
          perror("write bulk chunk"); tp_destroy(tp2); free(jobs2); goto fail_tbl; }
        hdr.comp_size += jobs2[j].comp_len;
        pool_release(out_pool, jobs2[j].comp);
      }
      pool_release(in_pool, jobs2[j].in_buf);
    }
    free(jobs2); tp_destroy(tp2);
  }
  ...
  if (do_idx) {
    ...
    for (uint32_t i=0;i<n;i++) {
      wix_entry_v1_t e={ .payload_off=table[i].offset, .orig_len=table[i].orig_len,
                         .comp_len=table[i].comp_len, .algo=table[i].algo };
  ...
  if (do_chk == WARP_CHK_XXH64) {
    ...
    const size_t bs=1<<20; unsigned char *buf=(unsigned char*)malloc(bs);
    int fd2=open(in_path,O_RDONLY);
    for(;;){ ssize_t r=read(fd2,buf,bs); if(r<0){perror("read chk");break;} if(r==0)break; XXH64_update(st,buf,(size_t)r); }
    close(fd2);
    unsigned long long d=XXH64_digest(st); XXH64_freeState(st); free(buf);
    fwrite(&d,8,1,fout);
  }
#endif
  wftr_footer_t ft={ .magic=WFTR_MAGIC, ._rsv=0, .wix_off=wix_off, .chk_off=chk_off };
  fwrite(&ft, sizeof(ft), 1, fout);

  if (opt->verbose)
    fprintf(stderr,"compressed %zu -> %llu bytes in %u chunks (locked algo=%d)\n",
      total, (unsigned long long)hdr.comp_size, n, prefer?prefer:locked_algo);

  free(table);
  pool_destroy(in_pool); pool_destroy(out_pool);
...
int warp_decompress_file(const char *in_path, const char *out_path, const warp_opts_t *opt) {
  int fd_in=open(in_path,O_RDONLY); if(fd_in<0){perror("open in"); return 1;}
  FILE *fin=fdopen(fd_in,"rb"); if(!fin){perror("fdopen"); close(fd_in); return 1;}

  warp_header_t hdr;
  if (fread(&hdr,sizeof(hdr),1,fin)!=1 || hdr.magic!=WARP_MAGIC || hdr.version!=WARP_VER) {
  ...
  FILE *fout=fopen(out_path,"wb+"); if(!fout){perror("fopen out"); free(table); fclose(fin); return 1;}
  (void)wc_truncate_file(fout, hdr.orig_size);
  ...
  if(!tp || !jobs){fprintf(stderr,"decomp OOM\n"); if(tp)tp_destroy(tp); free(jobs); pool_destroy(out_pool); free(table); fclose(fout); fclose(fin); return 1;}

  for (uint32_t i=0;i<hdr.chunk_count;i++){ jobs[i]=(d_job_t){ .fd=fd_in, .idx=(int)i, .ent=table[i], .out_pool=out_pool }; tp_submit(tp, do_decompress, &jobs[i]); }
  tp_barrier(tp);
  ...
  uint64_t off=0; int out_fd=fileno(fout);
...
#ifdef HAVE_XXHASH
  if(st){
    unsigned long long have=XXH64_digest(st); XXH64_freeState(st);
    wftr_footer_t ft2; fseek(fin,endpos-(long)sizeof(ft2),SEEK_SET); fread(&ft2,sizeof(ft2),1,fin);
    fseek(fin,(long)ft2.chk_off,SEEK_SET); wchk_header_t ch; fread(&ch,sizeof(ch),1,fin);
  ...
  free(table); fclose(fout); fclose(fin);
  return 0;
}
